# ChatBot 0.21 - Atualização
# Temas e nomes atribuidos com sucesso, as próximas versões irei editar
# o nome das sub-opções e trazer uma resposta quando selecionada.
...
def tratar_opcao_principal(opcao):
    if opcao == "Especialista Turismo":
        exibir_opcoes_sub_chatbot("Especialista Turismo", imagem_sub_chatbot)
    elif opcao == "Especialista MEI":
        exibir_opcoes_sub_chatbot("Especialista MEI", imagem_mei)
    elif opcao == "Especialista Agricultor":
        exibir_opcoes_sub_chatbot("Especialista Agricultor", imagem_especialista_agricultor)
    else:
...
def exibir_opcoes_sub_chatbot(nome_chatbot, imagem_sub):
    ...
    opcoes = [
        ...
        "Consultar FGTS",
        "Seguro desemprego",
        "Bolsa Família" if nome_chatbot != "Especialista Agricultor" else "MEI",  # Substituir a opção para evitar repetição
...
imagem_mei = ImageTk.PhotoImage(Image.open("C:/Users/Thiago/Desktop/Tutor GOV/W.png"))  # Caminho da imagem do sub-chatbot MEI
